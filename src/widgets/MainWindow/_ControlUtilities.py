...
from widgets.SpeedAxis import SpeedAxis
...
class ControlUtilities:
    ...
    def add_control_tab(self, name, channel):
        ...
        if control == "N/A" or controlType == "N/A":
            ...
            # Connections for Linear controls
            if control == "Linear":
                controlWidget = LinearAxis(controlID, feedback) # Instantiate LinearAxis widget
            ...
            # Connections for Speed controls
            elif control == "Speed":
                controlWidget = SpeedAxis(controlID) # Instantiate SpeedAxis widget
                controlWidget.axisWindowClosed.connect(self.window_to_tab)
                self.checkTimer.timeout.connect(self.manager.devices[name].check_connections)
                self.running.connect(self.manager.devices[name].set_running)
                self.manager.devices[name].updateRunningIndicator.connect(controlWidget.setRunningIndicator)
                ...
                if channel == 0:
                    ...
                    self.manager.devices[name].updatePositionProcessVariableC1.connect(controlWidget.setPositionProcessVariable)
                    self.manager.devices[name].updateFeedbackProcessVariableC1.connect(controlWidget.setFeedbackProcessVariable)
                    controlWidget.positiveJogRPMEnabled.connect(self.manager.devices[name].jog_rpm_positive_on_C1)
                    controlWidget.negativeJogRPMEnabled.connect(self.manager.devices[name].jog_rpm_negative_on_C1)
                    controlWidget.positiveJogRPMDisabled.connect(self.manager.devices[name].stop_command_C1)
                    controlWidget.negativeJogRPMDisabled.connect(self.manager.devices[name].stop_command_C1)
                elif channel == 1:
                    controlWidget.enable.connect(self.manager.devices[name].set_enable_C2)
                    controlWidget.PIDControl.connect(self.manager.devices[name].set_PID_control_C2)
                    controlWidget.KPChanged.connect(self.manager.devices[name].set_KP_C2)
                    ...
                    self.manager.devices[name].updatePositionProcessVariableC2.connect(controlWidget.setPositionProcessVariable)
                    self.manager.devices[name].updateFeedbackProcessVariableC2.connect(controlWidget.setFeedbackProcessVariable)
                    controlWidget.positiveJogRPMEnabled.connect(self.manager.devices[name].jog_rpm_positive_on_C2)
                    controlWidget.negativeJogRPMEnabled.connect(self.manager.devices[name].jog_rpm_negative_on_C2)
                    controlWidget.positiveJogRPMDisabled.connect(self.manager.devices[name].stop_command_C2)
                    controlWidget.negativeJogRPMDisabled.connect(self.manager.devices[name].stop_command_C2)
            
            controlWidget.controlType = control
